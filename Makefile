...
XILINX_SETTINGS_FILE = /opt/Xilinx/14.5/ISE_DS/settings64.sh
...
# Place and route: matches the FPGA components to actual instances, and connects them.
	...
	cd ${WORK_DIR} && \
	cp $^ . && \
	${PAR} -w -intstyle ise -ol high -mt 4 ${TOP_NAME}_map.ncd ${TOP_NAME}.ncd ${TOP_NAME}.pcf && \
...
# Map: maps netlist components to equivalent FPGA components within the available resources.
	...
	cd ${WORK_DIR} && \
	cp $^ . && \
	${MAP} -intstyle ise -p xc6slx45-csg324-2 -w -logic_opt off -ol high -t 1 -xt 0 -register_duplication off -r 4 -global_opt off -mt 4 -ir off -pr off -lc off -power off -o ${TOP_NAME}_map.ncd ${TOP_NAME}.ngd ${TOP_NAME}.pcf  && \
